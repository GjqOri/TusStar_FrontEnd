                    ...
                    <div class="col-lg-6 col-md-8 offset-md-2 offset-lg-3">
                        <el-form :rules="rules" :model="loginForm" class="signin-form">
                            <div class="form-group">
                                <label>用户名</label>
                                <input type="text" class="form-control" placeholder="请输入用户名" required v-model="loginForm.username">
                            </div>

                            <div class="form-group">
                                <label>密码</label>
                                <input type="password" class="form-control" placeholder="请输入密码" required v-model="loginForm.password">
                            </div>
                            <el-form-item  prop="code">
                                <el-input type="text" v-model="code"  > <template slot="append">
                            ...
                            <el-button type="primary" style="width: 100%;margin-top:30px;" @click="submit()">登陆</el-button>
                    ...
                    if (this.loginForm.username != '' && this.loginForm.password != '') {
                        this.axios.post(
                            ...
                            if(res.data.success==true) {
                                console.log("this is value :" + this.radio)

                                if (res.data.data === 'error_password') {
                                    alert("密码错误")
                                } else if (res.data.data === "error_no user" || res.data.data === "error_ no companyuser") {
                                    alert("用户不存在")
                                } else {
                                    Cookies.set("id",res.data.data)
                                    window.location = "/"
                                }
                                console.log("登录返回值" + res.data.data)
                              }
                            })
                            .catch(error => {
